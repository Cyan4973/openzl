...
/* DI_constant_numeric():
 * Decodes a constant numeric stream from serial input.
 * Header format: varint(nbElts)
 * Input: serial stream containing single element value
 * Output: numeric stream with nbElts copies of the value
 * Note: eltWidth is inferred from the input size.
 */
ZL_Report DI_constant_numeric(ZL_Decoder* dictx, const ZL_Input* ins[])
{
    ...
    /* Infer eltWidth from input size */
    size_t const eltWidth = ZL_Input_contentSize(in);
    ZL_RET_R_IF(
            corruption,
    ...
    /* Create output stream (type determined by graph definition) */
    ZL_Output* const out =
            ZL_Decoder_create1OutStream(dictx, dstNbElts, eltWidth);
    ZL_RET_R_IF_NULL(allocation, out);
    uint8_t* const outPtr = (uint8_t*)ZL_Output_ptr(out);
